...
class InceptionV4:
    ...
    def __model(self):
        ...
        # Top 5 Accuracy
        self.top_5_correct_prediction = tf.nn.in_top_k(self.probs, tf.argmax(self.y,1), 5)
        self.top_5_accuracy = tf.reduce_mean(tf.cast(self.top_5_correct_prediction, tf.float32))
    ...
    def calculate_loss(self, sess, Xt, yt, size=1000, step=10):
        ...
        size = Xt.shape[0] if Xt.shape[0] < size else size
    ...
    def predict_proba(self, sess, Xt, yt, size=1000, step=10):
        fc3ls = None
        sample_idx = random.sample(range(0, Xt.shape[0]), size)
        size = Xt.shape[0] if Xt.shape[0] < size else size
        for i in range(size / step):
            [fc3l] = sess.run([self.logits], feed_dict={self.X: Xt[sample_idx[i*step:(i+1)*step]], self.y: yt[sample_idx[i*step:(i+1)*step]]})
            self.__update_screen("\r{} out of {}".format((i+1)*step, size))
            if i == 0:
                fc3ls = fc3l
            else:
                fc3ls = np.vstack((fc3ls, fc3l))

        probs, accuracy = sess.run([self.probs, self.top_5_accuracy], feed_dict={self.logits: fc3ls, self.y: yt[sample_idx]})
